<template>
  <div class="space-y-8">
    ...
    <template v-else>
      <!-- 统计卡片 -->
      <section>
        <h2 class="h-section mb-4">站点统计</h2>
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div v-for="s in stats" :key="s.label" class="surface p-5 relative overflow-hidden">
            ...
            <div class="font-display font-bold leading-none tracking-tight" :class="s.color || 'text-fg'"
                 style="font-size: clamp(1.875rem, 3.5vw, 2.5rem);">
              {{ s.value }}
            </div>
            <div class="text-sm text-fg mt-2">{{ s.label }}</div>
        ...
        <div v-else class="space-y-4">
          ...
          <div class="surface p-6">
            <div class="flex items-baseline justify-between mb-3">
              <div class="text-sm font-medium text-fg">容器内存</div>
            ...
            <div class="flex items-baseline gap-3 mb-4">
              <span class="font-display font-bold tabular-nums tracking-tight" :class="memOver ? 'text-danger' : 'text-fg'"
                    style="font-size: clamp(2rem, 4.5vw, 2.75rem);">
            ...
            <!-- 进度条 -->
            <div class="h-1.5 bg-bg-0 rounded-full relative overflow-hidden">
              <div class="absolute inset-y-0 left-0 transition-all duration-500 rounded-full"
                   :class="memOver ? 'bg-danger' : (usageNum > 75 ? 'bg-warn' : 'bg-teal-300')"
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="surface p-4">
              <div class="text-xs text-fg-mute uppercase tracking-wider mb-2">Heap Alloc</div>
            ...
            <div class="surface p-4">
              <div class="text-xs text-fg-mute uppercase tracking-wider mb-2">Heap Sys</div>
            ...
            <div class="surface p-4">
              <div class="text-xs text-fg-mute uppercase tracking-wider mb-2">Released</div>
            ...
            <div class="surface p-4">
              <div class="text-xs text-fg-mute uppercase tracking-wider mb-2">Goroutines</div>
          ...
          <div class="surface p-4 flex flex-wrap gap-x-6 gap-y-2 font-mono text-xs">
            <div class="flex items-baseline gap-2">
              ...
              <span class="text-fg tabular-nums">{{ rt.numGC }}</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="text-fg-mute">FORCED</span>
              <span class="text-fg tabular-nums">{{ rt.numForcedGC }}</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="text-fg-mute">GC CPU</span>
              <span class="text-fg tabular-nums">{{ (rt.gcCpuFraction * 100).toFixed(2) }}%</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="text-fg-mute">GOMAXPROCS</span>
              <span class="text-fg tabular-nums">{{ rt.gomaxprocs }}</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="text-fg-mute">STACK</span>
              <span class="text-fg tabular-nums">{{ formatKiB(rt.stackInuseKiB) }}</span>
            </div>
          </div>
        </div>
      </section>
