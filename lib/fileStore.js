...
export const oauthStore = {
   ...
   *
   * ⚠️ 1.2.3 修复的安全 bug:
   *   原代码用 `throw new Error('REUSE_DETECTED')` 从 transaction 回调中抛错,
   *   但 better-sqlite3 的 transaction 语义是"callback 抛错 → 整个事务 ROLLBACK"。
   *   结果前一句 `UPDATE ... SET revoked = 1` 被回滚,攻击者重放**被检测到**但
   *   token 链**没被撤销** —— 攻击者可以继续用偷到的 token。违反 RFC-9700 §4.14。
   *   修复:callback 返回 status object 而不是抛错,transaction 正常 commit,
   *   保留 UPDATE 的效果;外层根据 status 决定返回哪种形态。
   */
  rotateRefreshToken({ oldRefreshToken, newTokenData }) {
    const txn = database.transaction(() => {
      ...
      // 重放检测:已 rotate 的 refresh_token 又送回来了
      // 整条 chain 全部撤销(合法客户端会被迫重新授权,比放行重放安全)
      if (row.replaced === 1) {
        const now = new Date().toISOString();
        database.prepare(
      ...
      if (row.revoked === 1) return { status: 'NOT_FOUND' };
      ...
      return {
        status: 'OK',
        id: newId, userId: row.userId,
        clientId: row.clientId, scope: row.scope,
      };
    });
    ...
    if (result.status === 'OK') {
      return {
        ...
        clientId: result.clientId, scope: result.scope,
      };
    }
...
if (typeof setInterval !== 'undefined') {
  ...
  if (!globalThis[G]) {
    ...
    // 1.2.0 调整节律:启动后 60 秒跑一次(清掉启动前就过期的),之后每 6 小时
    // 再跑一次 —— 原来的 24h 过长,高写入量表(activity_log / login_history 上
    // 面还没到 prune,别的表也是)会让 SQLite 持续膨胀。6 小时在"日清"与
    // "实时"之间取平衡,对 CPU 几乎无压力(单次 prune < 50ms)。
    setTimeout(run, 60_000).unref?.();
    const handle = setInterval(run, 6 * 3600_000);
    handle.unref?.();
    globalThis[G] = handle;
  }
}