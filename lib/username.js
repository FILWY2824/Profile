/**
 * lib/username.js —— 用户名与个人简介校验的单一数据源
 * ---------------------------------------------------------------------------
 * 之前用户名长度限制散落在 register / profile / admin users 三处,上一次要从
 * "2-32" 收紧到 "2-10" 还得改三个文件,极易漏改。从现在起所有需要校验用户
 * 名的地方都走 validateName(),确保修改只改一处。
 *
 * 同理,bio(个人简介)以前服务端没做任何长度校验,用户可以写一长篇 —— 这
 * 一轮加了 validateBio() 作为上限保护。
 *
 * 限制:
 *   • name: 长度 2–10 字符(去掉首尾空白后的可见长度;10 这个上限是产品约束,
 *     不是技术约束,改动请对齐前端 maxLength 与 DB 列宽如果有的话)
 *   • bio:  长度 0–200 字符,允许空字符串
 *   • 不允许全空白 / 纯空字符串(name,bio 不限)
...
/**
 * 校验个人简介(bio)。允许空字符串(视为"未设置"),非空时裁到 BIO_MAX_LENGTH。
 *
 * @param {unknown} raw
 * @returns {{ valid: true, value: string } | { valid: false, message: string }}
 */
export function validateBio(raw) {
  if (raw == null || raw === '') return { valid: true, value: '' };
  if (typeof raw !== 'string') {
    return { valid: false, message: '个人简介必须是字符串' };
  }
  const value = raw.trim();
  if (value.length > BIO_MAX_LENGTH) {
    return { valid: false, message: `个人简介长度不能超过 ${BIO_MAX_LENGTH} 个字符` };
  }
  return { valid: true, value };
}
