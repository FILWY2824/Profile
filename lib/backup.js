...
/**
 * 解析并校验 SFTP 目标主机,防止被用来渗内网或打本机。
 *
 * 攻击模型:admin 账户若被攻陷,攻击者可以在 /admin/settings 把 BACKUP_HOST
 * 改成 127.0.0.1、10.x、169.254.169.254(云元数据)或任意内网 IP,
 * 驱动本进程向这些目标建 SSH。轻则内网扫端口,重则直接把本机 DB 发到攻击者
 * 自己的 SFTP 服务器。
 *
 * 策略:
 *   • 默认拒绝私网/回环/链路本地目标。
 *   • 提供 BACKUP_ALLOW_PRIVATE_HOST=1 escape hatch,给确实要备份到 VPN/NAS
 *     的管理员一个显式开关 —— 这个开关只在 /admin/settings 改得了,
 *     攻击者要利用它必须先多开一个权限点,审计时也看得见。
 *   • 校验在"建连之前"做;DNS 解析一次,拿到所有 A/AAAA,任一落在私网就拒。
 */
async function assertBackupHostSafe(host) {
  if (!host || typeof host !== 'string') {
    throw new Error('备份服务器地址不能为空');
  }
  const allowPrivate = getSetting('BACKUP_ALLOW_PRIVATE_HOST') === '1';

  // 字面 IP
  if (net.isIP(host)) {
    if (isBlockedIp(host) && !allowPrivate) {
      throw new Error(`备份目标 IP ${host} 落在私网/回环/链路本地范围,已拒绝连接。如确需备份到内网 SFTP,请在 /admin/settings 开启 BACKUP_ALLOW_PRIVATE_HOST`);
    }
    return;
  }
  ...
  if (lower === 'localhost' || lower.endsWith('.localhost')) {
    if (!allowPrivate) throw new Error('备份目标指向 localhost,已拒绝');
  }
  if (lower === 'metadata' || lower === 'metadata.google.internal' || lower.endsWith('.internal')) {
    if (!allowPrivate) throw new Error(`备份目标主机名 ${host} 可疑,已拒绝`);
  }
  ...
  try {
    ...
    throw new Error(`备份服务器 DNS 解析失败:${err.code || err.message}`);
  }
  if (!records.length) {
    throw new Error('备份服务器 DNS 未返回任何记录');
  }
  for (const { address } of records) {
    if (isBlockedIp(address) && !allowPrivate) {
      throw new Error(`备份目标 ${host} 解析到 ${address},落在私网/回环/链路本地范围,已拒绝连接。如确需备份到内网 SFTP,请在 /admin/settings 开启 BACKUP_ALLOW_PRIVATE_HOST`);
    }
  }
}
 ...
 * 出错时 reject,不会泄漏连接。
 */
async function connectSsh(config, { signal } = {}) {
  // SSRF 守卫:先把主机校验通过,再建 TCP。
  await assertBackupHostSafe(config.host);
