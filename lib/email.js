 ...
 * HTML 转义 —— 纵深防御。
 *
 * 即使 SITE_NAME / 模板正文文本此刻都由管理员维护,仍然要走这一层:
 *   1) 管理员也可能被钓鱼/盗号,后台写入恶意内容后,邮件收件人打开会受影响
 *   2) 将来如果把其他字段(如用户名)嵌入邮件,有这一层就不用一处处手改
 *   3) 邮件客户端(尤其是 Outlook 桌面版)对 HTML 的容错和浏览器不完全一致,
 *      未转义的 `<` 可能导致排版错乱而不是简单显示
 *
 * 范围:&、<、>、"、' 这五个字符。这是 OWASP 推荐的 HTML 上下文最小转义集。
 *
 * 导出给测试用(tests/emailEscape.test.js)。
 */
...
function codeEmailHtml(siteName, title, body) {
  ...
  //
  // 注意:siteName 和 title 都走 escapeHtml —— body 由调用方确保已经是可信 HTML
  // (目前只有"验证码"这一句固定文案 + 数字,没有用户输入)。
        ...
        <table width="520" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;width:100%;background:#ffffff;border-radius:12px;border:1px solid #e5dfd9;overflow:hidden;">
              ...
              <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                  ...
                  <td style="vertical-align:middle;">
                    <div style="font-size:1.1rem;font-weight:600;color:#1a1612;letter-spacing:-0.01em;line-height:1.1;">${s}</div>
          ...
          <tr>
            <td style="padding:28px 32px 8px;">
              <h2 style="margin:0 0 12px;color:#1a1612;font-size:1.2rem;font-weight:600;">${t}</h2>
...
export async function sendVerificationCode(email, code) {
  ...
  // 正文里插 siteName 的位置也要转义 —— 模板字符串是我们构造的 HTML,任何来自
  // settings 的文本在嵌入前都得过 escapeHtml(验证码 `code` 本身由服务端生成的
  // 6 位数字,不需要转义,但为了一致性也包一下,几乎零开销)。
  const safeName = escapeHtml(cfg.siteName);
  const safeCode = escapeHtml(code);
  const html = codeEmailHtml(
    cfg.siteName,
    '邮箱验证码',
    `您正在注册 ${safeName} 账号,验证码为:<br>
     <span style="font-size:2rem;font-weight:700;color:#1a1612;letter-spacing:0.2em;">${safeCode}</span><br>
