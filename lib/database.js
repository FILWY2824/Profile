...
{ version: 4, up(db) {
  const staleKeys = [
    ...
    'BACKUP_PASSWORD',
    'BACKUP_PRIVATE_KEY',
    'BACKUP_PRIVATE_KEY_PASSPHRASE',
    'BACKUP_REMOTE_DIR',
    'BACKUP_HISTORY_KEEP',
    'BACKUP_ALLOW_PRIVATE_HOST',
  ];

  db.exec(`
    DROP TABLE IF EXISTS backup_jobs;
    DROP INDEX IF EXISTS idx_backup_jobs_started;
    DROP INDEX IF EXISTS idx_backup_jobs_status;
  `);

  const placeholders = staleKeys.map(() => '?').join(',');
  db.prepare(`DELETE FROM settings WHERE key IN (${placeholders})`).run(...staleKeys);
} },
];
